...
model = Sequential()
model.add(Dense(1, input_dim=75, activation = 'linear'))

#reduce_lr = ReduceLROnPlateau(monitor="val_loss", factor=0.1, patience=35, min_lr=1e-6, verbose=1)

model.compile(optimizer='sgd', loss='mean_squared_error', metrics=[r2_score], run_eagerly=True)
